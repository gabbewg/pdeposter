...
\subsection*{Steg 1: Val av integrerande faktor}

Strategin är densamma som för linjära ordinära differentialekvationer: vi söker en transformation som eliminerar driften. Sätt
\begin{equation}
...
\begin{remark}
Motivationen är att om man försummar bruset reduceras \eqref{eq:ou-sde} till ODE:n $\dot X = \theta(\mu - X)$ med lösning $X_t - \mu = (x_0 - \mu)e^{-\theta t}$. Storheten $e^{\theta t}(X_t - \mu)$ är då \emph{konstant}. Med brus förväntar vi oss att $Y_t$ blir en ren martingal --- ingen drift.
\end{remark}
...
Genom transformationen $Y_t = e^{\theta t}(X_t - \mu)$, som elimineras driften via Itôs lemma, har vi härlett att OU-processen ges av
\[
